...
def set_page_style() -> None:
    ...
    st.markdown(
        ...
        .dvs-chip.primary {{ background: linear-gradient(90deg, var(--cb-blue), #0066ff); border-color: rgba(0,0,0,0); }}
        .dvs-hero {{
...
def hero() -> None:
    ...
    st.markdown(
        ...
        <div class="dvs-hero">
            <h1>⚽ {APP_NAME}</h1>
            <p>{APP_VERSION} · Screening · Scouting · Team Screening · Contact Lock · Club Brugge reports · PDF</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def main() -> None:
    ...
    st.markdown(
        """
        <div class="dvs-card">
            <b>v11: drie workflows.</b><br/>
            <b>Screening</b> = goedkope eerste analyse van één speler.<br/>
            <b>Scouting</b> = individueel rapport met positie-template.<br/>
            <b>Team Screening</b> = wedstrijdscan met korte screening per speler en shortlist/rewatch-advies.
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    if report_type == "Team Screening":
        has_required_input = video_file is not None and (bool(team_lineups) or bool(team_a_name.strip()) or bool(team_b_name.strip()))
        start_label = "▶ Start Team Screening"
    else:
        has_required_input = video_file is not None and bool(player_name.strip()) and bool(shirt_number.strip())
