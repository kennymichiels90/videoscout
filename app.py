...
APP_VERSION = "v16 Drive Real Fix"
...
def google_drive_direct_url(url: str) -> str:
    """Convert common Google Drive share links to a more reliable download endpoint."""
    raw = (url or "").strip()
    file_id = extract_google_drive_file_id(raw)
    if file_id:
        return f"https://drive.usercontent.google.com/download?id={file_id}&export=download&confirm=t"
    return raw


def html_error_message(html_bytes: bytes) -> str:
    sample = html_bytes[:12000].decode("utf-8", errors="ignore").lower()
    if "accounts.google.com" in sample or "sign in" in sample or "inloggen" in sample:
        return "Google Drive vraagt om in te loggen. Zet het bestand op 'Iedereen met de link kan bekijken'."
    if "access denied" in sample or "toegang geweigerd" in sample or "you need access" in sample or "request access" in sample:
        return "Geen toegang tot dit Google Drive-bestand. Zet delen op 'Iedereen met de link kan bekijken'."
    if "quota" in sample or "download quota" in sample or "too many users" in sample:
        return "Google Drive blokkeert de download tijdelijk door quota/limiet. Probeer later, maak een kopie, of gebruik upload/directe downloadlink."
    if "virus scan" in sample or "can't scan this file" in sample or "download anyway" in sample:
        return "Google Drive toont een grote-bestanden/virus-scan bevestigingspagina. De app kon die niet automatisch passeren. Probeer opnieuw met een publieke link, of gebruik upload/directe downloadlink."
    return "De link leverde een HTML-pagina op in plaats van een videobestand. Controleer delen/toegang of gebruik een directe downloadlink."


def validate_downloaded_video_file(tmp_path: str) -> None:
    if not Path(tmp_path).exists() or Path(tmp_path).stat().st_size == 0:
        raise RuntimeError("Download mislukt: er werd geen videobestand gevonden.")
    with open(tmp_path, "rb") as f:
        head = f.read(12000)
    lower = head.lower()
    if b"<html" in lower or b"<!doctype html" in lower:
        raise RuntimeError(html_error_message(head))


def download_with_gdown(url: str, tmp_path: str) -> bool:
    if gdown is None:
        return False
    try:
        file_id = extract_google_drive_file_id(url)
        if file_id:
            result = gdown.download(id=file_id, output=tmp_path, quiet=True, fuzzy=True, use_cookies=False)
        else:
            result = gdown.download(url=url, output=tmp_path, quiet=True, fuzzy=True, use_cookies=False)
        return bool(result) and Path(tmp_path).exists() and Path(tmp_path).stat().st_size > 0
    except Exception:
        return False


def download_with_requests(url: str, tmp_path: str, max_mb: int = UPLOAD_LIMIT_MB) -> None:
    direct_url = google_drive_direct_url(url) if is_google_drive_url(url) else url
    session = requests.Session()
    headers = {
        "User-Agent": "Mozilla/5.0",
        "Accept": "*/*",
    }
    response = session.get(direct_url, stream=True, timeout=90, allow_redirects=True, headers=headers)
    ...
    if token:
        ...
        response.close()
        response = session.get(f"{direct_url}{sep}confirm={token}", stream=True, timeout=90, allow_redirects=True, headers=headers)
...
def download_video_url_to_temp(url: str, max_mb: int = UPLOAD_LIMIT_MB) -> Tuple[str, str]:
    """Download a public/direct video link to a temp file.
    For Google Drive, try gdown first and requests fallback second.
    """
    raw_url = (url or "").strip()
    if not raw_url:
        raise RuntimeError("Geen videolink ingevuld.")
    ...
    try:
        ...
        if is_google_drive_url(raw_url):
            downloaded = download_with_gdown(raw_url, tmp_path)
            if downloaded:
                validate_downloaded_video_file(tmp_path)

        if not downloaded:
            download_with_requests(raw_url, tmp_path, max_mb=max_mb)
            validate_downloaded_video_file(tmp_path)

        return tmp_path, google_drive_direct_url(raw_url) if is_google_drive_url(raw_url) else raw_url
    except Exception:
        try:
            os.remove(tmp_path)
        except Exception:
...
def main() -> None:
    ...
    if video_input_mode == "Upload":
        ...
        video_url = st.text_input(
            ...
            help="Zet je Google Drive-bestand op: Iedereen met de link kan bekijken. De app gebruikt een aparte Drive-downloader voor grote bestanden.",
        )
        st.caption("Tip: gebruik de gewone Google Drive share-link. Het bestand moet gedeeld zijn met ‘Iedereen met de link kan bekijken’. Voor zeer grote bestanden blijft Streamlit Cloud afhankelijk van geheugen en verwerkingstijd.")
