...
MAX_VIDEO_BYTES = UPLOAD_LIMIT_MB * 1024 * 1024
...
APP_NAME = "Club Brugge Video Scout"
APP_VERSION = ""
...
def set_page_style() -> None:
    ...
    st.markdown(
        ...
        :root {{
            --cb-bg: #030916;
            --cb-bg-2: #071222;
            --cb-surface: rgba(7, 18, 34, 0.92);
            --cb-surface-2: rgba(10, 27, 48, 0.9);
            --cb-line: rgba(68, 138, 255, 0.26);
            --cb-blue: #0d78ff;
            --cb-blue-2: #1890ff;
            --cb-text: #eef5ff;
            --cb-muted: #9fb3cf;
        ...
        .stApp {{
            background:
                linear-gradient(180deg, rgba(1,7,16,0.90) 0%, rgba(2,8,18,0.96) 22%, rgba(3,9,22,1) 100%),
                url('{bg_uri}') center top / cover no-repeat fixed;
            color: var(--cb-text);
        ...
        .dvs-topnav {{
            ...
            padding: 10px 2px 18px 2px; margin-bottom: 8px;
            border-bottom: 1px solid rgba(255,255,255,0.08);
        }}
        .dvs-brand {{ display:flex; align-items:center; gap:14px; }}
        .dvs-brand img {{ width: 54px; height: 54px; object-fit: contain; }}
        .dvs-brand-text {{ line-height: 1.05; }}
        .dvs-brand-text .small {{ color: var(--cb-muted); font-size: 0.78rem; text-transform: uppercase; letter-spacing: 0.16em; }}
        .dvs-brand-text .big {{ font-family: 'Barlow Condensed', sans-serif; font-size: 1.7rem; text-transform: uppercase; font-weight: 700; }}
        .dvs-topchips {{ display:flex; gap:10px; flex-wrap:wrap; justify-content:flex-end; }}
        .dvs-chip {{
        ...
        .dvs-chip.active, .dvs-chip.primary {{ background: linear-gradient(90deg, var(--cb-blue), #0066ff); border-color: rgba(0,0,0,0); color:#021024; font-weight:700; }}
        .dvs-hero {{
            position: relative; overflow: hidden;
            padding: 34px 34px 28px 34px; border-radius: 30px;
            background:
                linear-gradient(90deg, rgba(2, 10, 21, 0.95) 0%, rgba(5, 18, 34, 0.87) 45%, rgba(7, 29, 56, 0.78) 70%, rgba(9, 49, 101, 0.78) 100%),
        ...
        .dvs-hero::after {{
            ...
            background: linear-gradient(135deg, rgba(13,120,255,0.0) 15%, rgba(13,120,255,0.10) 60%, rgba(13,120,255,0.18) 100%);
            pointer-events:none;
        }}
        .dvs-hero-inner {{ position:relative; z-index:1; display:flex; justify-content:space-between; gap:16px; align-items:flex-start; }}
        .dvs-hero-left {{ max-width: 720px; }}
        .dvs-kicker {{ color:#8fbefc; font-size:0.88rem; letter-spacing:0.20em; text-transform:uppercase; margin-bottom:10px; }}
        .dvs-hero h1 {{ margin:0; line-height:0.94; font-size:3.5rem !important; max-width: 8ch; }}
        .dvs-hero p {{ margin: 12px 0 0 0; color:#dde8fa; font-size:1rem; max-width: 72ch; }}
        .dvs-hero-right {{ display:flex; flex-direction:column; align-items:flex-end; gap:14px; min-width:240px; }}
        .dvs-hero-logo {{ width:96px; height:96px; object-fit:contain; filter: drop-shadow(0 6px 12px rgba(0,0,0,0.35)); }}
        .dvs-pillrow {{ display:flex; gap:10px; flex-wrap:wrap; justify-content:flex-end; }}
        .dvs-pill {{ padding:10px 16px; border-radius:999px; background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.15); color:#fff; font-size:0.9rem; text-transform:uppercase; letter-spacing:0.04em; }}
        .dvs-pill.primary {{ background: linear-gradient(90deg, var(--cb-blue), var(--cb-blue-2)); border-color: transparent; font-weight:600; }}
        .dvs-card {{
            padding: 18px 20px; border-radius: 22px;
            background: linear-gradient(180deg, rgba(6,17,33,0.90) 0%, rgba(6,17,33,0.97) 100%);
            border: 1px solid rgba(255,255,255,0.08);
            box-shadow: 0 8px 24px rgba(0,0,0,0.18);
            margin-bottom: 14px;
        }}
        .dvs-card strong, .dvs-card b {{ color:#fff; }}
        .dvs-warning {{ padding: 14px 16px; border-radius: 16px; background: rgba(250, 204, 21, 0.10); border: 1px solid rgba(250, 204, 21, 0.35); margin-bottom: 14px; }}
        .dvs-ok {{ padding: 14px 16px; border-radius: 16px; background: rgba(34, 197, 94, 0.11); border: 1px solid rgba(34, 197, 94, 0.35); margin-bottom: 14px; }}
        .dvs-small {{ opacity: 0.74; font-size: 0.88rem; color: var(--cb-muted); }}
        [data-testid="stSidebar"] {{ background: linear-gradient(180deg, #050d19 0%, #091629 100%); border-right: 1px solid rgba(255,255,255,0.07); }}
        [data-testid="stSidebar"] * {{ color: #eef5ff; }}
        .stButton > button {{
            border-radius: 999px; font-weight: 700; min-height: 46px;
            background: linear-gradient(90deg, var(--cb-blue), #0066ff); color:#fff; border:none;
            box-shadow: 0 10px 24px rgba(13,120,255,0.22);
        }}
        .stButton > button[kind="secondary"] {{
            background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.18); box-shadow:none;
        }}
        ...
        .stNumberInput input {{
            background: rgba(255,255,255,0.03) !important; border-radius: 16px !important; color:#fff !important;
        }}
        ...
        .stExpander {{ background: rgba(255,255,255,0.02); border-radius: 18px; border: 1px solid rgba(255,255,255,0.06); }}
        div[data-testid="stFileUploader"] section {{
            background: rgba(255,255,255,0.03); border: 1px dashed rgba(24,144,255,0.45); border-radius: 20px;
        }}
        div[data-testid="stMetric"] {{ background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); border-radius: 18px; padding: 10px 12px; }}
        .stDataFrame, .stTable {{ background: rgba(255,255,255,0.03); border-radius: 18px; }}
        @media (max-width: 900px) {{
            .dvs-hero-inner, .dvs-topnav {{ flex-direction: column; align-items:flex-start; }}
            .dvs-hero-right, .dvs-topchips, .dvs-pillrow {{ justify-content:flex-start; align-items:flex-start; }}
            .dvs-hero h1 {{ font-size: 2.6rem !important; }}
        }}
...
def hero() -> None:
    ...
    st.markdown(
        f"""
        <div class="dvs-topnav">
            <div class="dvs-brand">
                <img src="{logo_uri}" alt="Club Brugge logo" />
                <div class="dvs-brand-text">
                    <div class="small">Club Brugge</div>
                    <div class="big">Video Scout</div>
                </div>
            </div>
            <div class="dvs-topchips">
                <div class="dvs-chip active">Screening</div>
                <div class="dvs-chip">Scouting</div>
                <div class="dvs-chip">Team Screening</div>
            </div>
        </div>
        <div class="dvs-hero">
            <div class="dvs-hero-inner">
                <div class="dvs-hero-left">
                    <div class="dvs-kicker">AI Assisted Video Scouting</div>
                    <h1>Video Scout</h1>
                    <p>Upload of importeer wedstrijdbeelden, herken spelers via Player Lock en genereer screenings of scoutingrapporten in een strakke Club Brugge-stijl.</p>
                </div>
                <div class="dvs-hero-right">
                    <img class="dvs-hero-logo" src="{logo_uri}" alt="Club Brugge logo" />
                    <div class="dvs-pillrow">
                        <div class="dvs-pill primary">Start analyse</div>
                        <div class="dvs-pill">Video tot 3 GB via link</div>
                        <div class="dvs-pill">PDF</div>
                    </div>
                </div>
            </div>
        </div>
...
def normalize_video_url(url: str) -> str:
    ...
    if "dropbox.com" in url:
        url = url.replace("?dl=0", "?dl=1").replace("?dl=", "?dl=")
        if "dl=1" not in url:
            sep = "&" if "?" in url else "?"
            url = f"{url}{sep}dl=1"
    # Google Drive file links are handled by gdown in download_video_url_to_temp.
    return url


def filename_from_headers_or_url(url: str, headers: Dict[str, str]) -> str:
    cd = headers.get("content-disposition", "") or headers.get("Content-Disposition", "")
    match = re.search(r'filename\*?=(?:UTF-8\'\')?"?([^";]+)', cd)
    if match:
        return re.sub(r"[^A-Za-z0-9._ -]+", "_", match.group(1).strip())[:120] or "downloaded_video.mp4"
...
def suffix_for_video_name(name: str) -> str:
...
def download_video_url_to_temp(url: str) -> Tuple[str, str]:
    """Download a large match video server-side, avoiding browser upload limits."""
    url = normalize_video_url(url)
    if not url.lower().startswith(("http://", "https://")):
        raise RuntimeError("Geef een geldige http(s)-link naar een videobestand.")

    # Google Drive needs cookie/confirm-token handling; gdown is more reliable for large files.
    if "drive.google.com" in url:
        try:
            import gdown  # type: ignore
        except Exception as exc:
            raise RuntimeError("Google Drive-link herkend, maar gdown is niet geïnstalleerd. Check requirements.txt.") from exc
        with tempfile.NamedTemporaryFile(delete=False, suffix=".mp4") as tmp:
            tmp_path = tmp.name
        result = gdown.download(url, tmp_path, quiet=True, fuzzy=True)
        if not result or not Path(tmp_path).exists() or Path(tmp_path).stat().st_size == 0:
            raise RuntimeError("Google Drive-video kon niet gedownload worden. Zet de link op 'Iedereen met de link kan bekijken' of gebruik een directe downloadlink.")
        if Path(tmp_path).stat().st_size > MAX_VIDEO_BYTES:
            os.remove(tmp_path)
            raise RuntimeError(f"Video is groter dan {UPLOAD_LIMIT_MB} MB.")
        return tmp_path, "google_drive_video.mp4"

    headers = {"User-Agent": "Mozilla/5.0"}
    with requests.get(url, stream=True, allow_redirects=True, headers=headers, timeout=(15, 90)) as r:
        r.raise_for_status()
        ...
        if total and total > MAX_VIDEO_BYTES:
            raise RuntimeError(f"Video is {round(total / 1024 / 1024)} MB en dus groter dan {UPLOAD_LIMIT_MB} MB.")
        filename = filename_from_headers_or_url(url, r.headers)
        suffix = suffix_for_video_name(filename)
        with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as tmp:
            ...
            for chunk in r.iter_content(chunk_size=1024 * 1024):
                ...
                if downloaded > MAX_VIDEO_BYTES:
                    tmp.close()
                    os.remove(tmp_path)
                    raise RuntimeError(f"Video overschrijdt {UPLOAD_LIMIT_MB} MB tijdens download.")
                tmp.write(chunk)
    return tmp_path, filename


def open_video_to_temp(video_file: Any) -> Tuple[str, Dict[str, Any]]:
    # Supports: Streamlit uploaded file OR server-side URL string OR dict with path/name.
    if isinstance(video_file, str):
        tmp_path, filename = download_video_url_to_temp(video_file)
        source = "url"
    elif isinstance(video_file, dict) and video_file.get("path"):
        tmp_path = str(video_file["path"])
        filename = str(video_file.get("name") or Path(tmp_path).name)
        source = str(video_file.get("source") or "path")
    else:
        filename = getattr(video_file, "name", "uploaded_video.mp4")
    ...
    if not cap.isOpened():
        try:
            ...
            pass
        raise RuntimeError("Video kon niet geopend worden. Gebruik bij voorkeur mp4/h264 of een directe downloadlink.")
...
def build_pdf(
    ...
    story.append(pdf_paragraph(f"Rapportdatum: {datetime.now().strftime('%Y-%m-%d %H:%M')} · Contact Lock + video frame-sampling", small))
...
def build_team_screening_pdf(
    ...
    story.append(pdf_paragraph(f"Rapportdatum: {datetime.now().strftime('%Y-%m-%d %H:%M')} · Team Screening", small))
...
def main() -> None:
    ...
    st.markdown(
        """
        <div class="dvs-card">
            <b>Kies je workflow.</b><br/>
            Screening voor een snelle eerste indruk, Scouting voor een individueel positierapport, Team Screening voor een shortlist uit een volledige wedstrijd.
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.caption("Voor grote wedstrijdbeelden is een videolink aanbevolen. Dan downloadt de app de video server-side en bots je niet op de 200 MB browser-uploadlimiet.")
    video_source_mode = st.radio(
        "Videobron",
        ["Videolink", "Upload"],
        horizontal=True,
        help="Gebruik Videolink voor grote bestanden tot ongeveer 3 GB. Upload blijft beschikbaar voor korte clips.",
    )
    video_file = None
    video_url = ""
    if video_source_mode == "Upload":
        video_file = large_file_uploader(
            "Upload wedstrijd of geknipte beelden",
            type=["mp4", "mov", "m4v", "avi", "mkv"],
            help="Als je hier nog 200 MB ziet: gebruik Videolink of zet .streamlit/config.toml in de root van je repo en reboot de app.",
        )
        limit_mb = active_upload_limit_mb()
        if limit_mb and limit_mb < UPLOAD_LIMIT_MB:
            st.warning("Upload via browser blijft hier beperkt tot 200 MB. Gebruik bovenaan 'Videolink' voor grote wedstrijden.")
        else:
        ...
        video_url = st.text_input(
            "Videolink",
            value="",
            placeholder="Plak een directe mp4/mov-downloadlink, Dropbox-link of Google Drive-link",
            help="De app downloadt de video naar de server en verwerkt daarna lokaal de frames. Zorg dat de link toegankelijk is voor iedereen met de link.",
        )
        st.markdown(
            """
            <div class="dvs-card dvs-small">
                Aanbevolen voor grote wedstrijden: Dropbox met <b>dl=1</b>, Google Drive met 'iedereen met de link kan bekijken', of een directe mp4-link. Richtlimiet: 3 GB.
            </div>
            """,
            unsafe_allow_html=True,
        )
    video_input = video_url.strip() if video_source_mode == "Videolink" else video_file
    ...
    st.caption(f"Gekozen modus: **{report_type}** · standaardmodel: **{defaults['model']}**.")

    if report_type == "Team Screening":
        has_required_input = bool(video_input) and (bool(team_lineups) or bool(team_a_name.strip()) or bool(team_b_name.strip()))
        start_label = "▶ Start Team Screening"
    else:
        has_required_input = bool(video_input) and bool(player_name.strip()) and bool(shirt_number.strip())
